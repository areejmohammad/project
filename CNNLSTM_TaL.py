...
from keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, Activation, InputLayer, Dropout, Input, LSTM, TimeDistributed
...
for speaker in speakers:
    ...
    for train_valid in ['train', 'valid']:
        ...
        ult[train_valid], melspec[train_valid] = create_dataset_img(ult[train_valid], melspec[train_valid], look_back = n_sequence)
    ...
    model.add(TimeDistributed(Conv2D(filters=30,kernel_size=(13,13),strides=(2,2),activation="swish", padding="same",kernel_initializer=keras.initializers.he_uniform(seed=None), kernel_regularizer=regularizers.l1(0.00001), input_shape=ult['train'].shape[1:])))
    model.add(TimeDistributed(Dropout(0.2))) 
    model.add(TimeDistributed(Conv2D(filters=60,kernel_size=(13,13),strides=(2,2),activation="swish", padding="same",kernel_initializer=keras.initializers.he_uniform(seed=None) ,kernel_regularizer=regularizers.l1(0.00001))))
    model.add(TimeDistributed(Dropout(0.2))) 
    model.add(TimeDistributed(MaxPooling2D(pool_size=(2,2))))
    model.add(TimeDistributed(Conv2D(filters=90,kernel_size=(13,13),strides=(2,2),activation="swish", padding="same",kernel_initializer=keras.initializers.he_uniform(seed=None), kernel_regularizer=regularizers.l1(0.00001))))
    model.add(TimeDistributed(Dropout(0.2))) 
    model.add(TimeDistributed(Conv2D(filters=120,kernel_size=(13,13),strides=(1,1),activation="swish", padding="same",kernel_initializer=keras.initializers.he_uniform(seed=None), kernel_regularizer=regularizers.l1(0.00001))))
    model.add(TimeDistributed(Dropout(0.2))) 
    model.add(TimeDistributed(MaxPooling2D(pool_size=(2,2))))
    ...
    print(current_date)
    model_name = 'models/UTI_to_STFT_CNN-improved_LSTM' + speaker + '_' + current_date
    
    # callbacks
    earlystopper = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=3, verbose=1, mode='auto')
    lrr = ReduceLROnPlateau(monitor='val_loss', patience=2, verbose=1, factor=0.5, min_lr=0.0001) 
